...
from sqlalchemy import Column, String, Float, DateTime, JSON, Text, ForeignKey, BigInteger, Integer, Boolean
...
class EnterpriseBroadbandFaultSummary(Base):
    """企宽故障率 - 横山汇总指标（来自"报表"sheet）"""
    __tablename__ = "enterprise_broadband_fault_summary"

    id = Column(BigInteger, primary_key=True)
    report_date = Column(String(50), comment="通报日期，如 2026-06-04")
    district = Column(String(50), default="横山", comment="区县")
    latest_filename = Column(String(255), comment="报表文件名")
    fault_rate = Column(String(50), comment="企宽故障率测算")
    fault_count = Column(String(50), comment="采集故障次数")
    total_alarm_duration = Column(String(50), comment="累计告警时长")
    unrecoverd_work_orders = Column(String(50), comment="企宽未恢复告警工单")
    created_at = Column(DateTime(timezone=True), default=lambda: datetime.now(timezone.utc))


class EnterpriseBroadbandFaultRecord(Base):
    """企宽故障率 - 横山故障明细（来自"企宽故障率月清单"sheet）"""
    __tablename__ = "enterprise_broadband_fault_record"
    ...
    account = Column(String(100), comment="账号")
    alarm_total = Column(String(50), comment="告警累计")
